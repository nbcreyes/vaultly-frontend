      ...
      <div class="space-y-6">
        ...
        <div class="card p-6 space-y-4">
          ...
          <div>
            ...
            <input v-model="form.title" type="text" class="input" :class="{ 'input-error': errors.title }" placeholder="My Awesome UI Kit" />
            <p v-if="errors.title" class="error-message">{{ errors.title }}</p>
          </div>

          <div>
            <label class="label">Short description</label>
            <input v-model="form.short_description" type="text" class="input" placeholder="One line summary shown in cards" maxlength="160" />
          </div>

          <div>
            <label class="label">Full description <span class="text-red-500">*</span></label>
            <textarea
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <select v-model="form.category_id" class="input" :class="{ 'input-error': errors.category_id }">
            ...
            <div>
              <label class="label">License <span class="text-red-500">*</span></label>
          ...
          <div>
            ...
            <input v-model="tagsInput" type="text" class="input" placeholder="vue, tailwind, dashboard" />
          </div>
        </div>

        <!-- Product file -->
        <div class="card p-6">
          <h2 class="font-semibold text-gray-900 mb-4">Product File <span class="text-red-500">*</span></h2>
          <p class="text-sm text-gray-500 mb-3">The file buyers will download. ZIP, PDF, MP3, MP4, fonts accepted. Max 200MB.</p>

          <div
            class="border-2 border-dashed rounded-xl p-8 text-center transition-colors"
            :class="productFile ? 'border-brand-300 bg-brand-50' : 'border-gray-200 hover:border-brand-300'"
            @dragover.prevent
            @drop.prevent="handleFileDrop"
          >
            <template v-if="productFile">
              <svg class="w-10 h-10 text-brand-500 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
              </svg>
              <p class="text-sm font-medium text-gray-900">{{ productFile.name }}</p>
              <p class="text-xs text-gray-400 mt-1">{{ formatFileSize(productFile.size) }}</p>
              <button @click="productFile = null" class="text-xs text-red-500 mt-2 hover:text-red-700">Remove</button>
            </template>
            <template v-else>
          ...
          <p v-if="errors.product_file" class="error-message mt-2">{{ errors.product_file }}</p>
        </div>

        <!-- Preview images -->
        <div class="card p-6">
          <h2 class="font-semibold text-gray-900 mb-1">Preview Images</h2>
          <p class="text-sm text-gray-500 mb-4">Upload up to 5 images showing your product. First image is the thumbnail.</p>
          ...
          <div class="grid grid-cols-3 gap-3 mb-3">
            ...
            >
              ...
              >×</button>
              <span v-if="i === 0" class="absolute bottom-1 left-1 bg-black bg-opacity-50 text-white text-xs px-1.5 py-0.5 rounded">Cover</span>
            </div>
            ...
            >
              <input type="file" accept="image/*" multiple class="hidden" @change="addPreviewImages" />
        ...
        <div class="flex gap-3 pb-8">
          <button @click="submit('draft')" :disabled="saving" class="btn-secondary flex-1">
            {{ saving === 'draft' ? 'Saving...' : 'Save as draft' }}
          </button>
          <button @click="submit('published')" :disabled="saving" class="btn-primary flex-1">
